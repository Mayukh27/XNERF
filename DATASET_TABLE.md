...
- **Architecture inference is filename-substring matching, not binary
  analysis.** `infer_arch()` (`build_dataset.py`) checks tokens like
  `"arm64"`, `"mipsel"`, and `"x64"` against the lower-cased file path and
  now returns the explicit `unknown` label when no match exists. For
  non-PE/ELF feature-CSV/Parquet datasets (the overwhelming majority of
  supported sources), there is often no real architecture signal in the
  data at all, so `arch="unknown"` is the scientifically correct default.
  `xnerf/datasets/audit.py` (`architecture_audit_report`,
  `detect_single_architecture_dataset`) remains the built-in self-check for
  single-architecture degeneracy. Unknown rows are still used for
  malware/family learning, but they are masked from architecture
  supervision.
...
is now wired into the model through `ISREncoder`. Unknown architecture
rows are kept as `unknown` and masked from architecture supervision. None
of the seven dataset sources actually ingested by `build_dataset.py`
provide verified multi-architecture binary samples in the repository; the
six-way architecture coverage is therefore a **capability of the
disassembly module plus the unknown-label fallback**, not a demonstrated
property of any assembled training set.
